...
|:---|---:|---:|
| **Total packages** | **1441** |  |
| &ensp;Purelib (pure Python) | 1198 | 83.1% |
| &ensp;Platlib (native code) | 243 | 16.9% |
| &ensp;Manylinux + bundleable | 176 | 12.2% |
| &ensp;&ensp;Manylinux-only | 165 | 11.5% |
| &ensp;&ensp;Could be bundled | 9 | 0.6% |
| &ensp;&ensp;Pre-built (manylinux) | 2 | 0.1% |
| &ensp;Platform-dependent | 62 | 4.3% |
...
| **Purelib + manylinux + bundleable** | **1374** | **95.4%** |
| **Platform/accel + other** | **67** | **4.6%** |
...
83 unique libraries across 236 project references
...
| torchcodec | libavcodec.so.60, libavdevice.so.60, libavfilter.so.9, libavformat.so.60, libavutil.so.58, libc10.so, libc10_cuda.so, libcudart.so.12, libnvrtc.so.12, libpython3.12.so.1.0, libswresample.so.4, libswscale.so.7, libtorch.so, libtorch_cpu.so, libtorch_cuda.so |
...
**Total:** 236 packages with ELF dependencies (165 manylinux-only, 9 bundleable, 19 accelerator, 42 unbundleable, 1 undecided, 0 unknown)
...
**Pre-built manylinux (2):** nvidia-cutlass-dsl-libs-base, soundfile

**Other (7):** dulwich, eval-hub-server, frozendict, mysql-connector-python, pyyaml, rtree, torch
